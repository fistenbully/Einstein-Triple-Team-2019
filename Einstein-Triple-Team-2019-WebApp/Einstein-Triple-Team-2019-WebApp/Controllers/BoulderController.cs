...
namespace Einstein_Triple_Team_2019_WebApp.Controllers
{
    ...
    public class BoulderController : ControllerBase
    {
        ...
        public static IEnumerable<object> LB(IDbConnection dbConnection)
        {
            ...
            var b = vbg
            .Select(v => new { Team = v.TeamName, v.SexMultiplier, Boulder = v.BoulderName, Flash = v.Flash ? 1 : 0, Top = v.Top ? 1 : 0 })
            .GroupBy(v => v.Team)
            .Select(v => new { Team = v.Key, Points = v.Sum(l => l.Flash * 15 + l.Top * 10) * v.FirstOrDefault().SexMultiplier, Flash = v.Sum(l => l.Flash), Top = v.Sum(l => l.Top) })
